...
class DashboardController extends Controller
{
    ...
    public function saveRentReminder(Request $request)
    {
        ...
        try {
            $tenantUser = auth()->user()->tenant;
            $tenantUser->rent_reminder_date = $request->rent_reminder_date;
            $tenantUser->save();

            return response()->json(['success' => true, 'message' => 'Rent reminder date saved successfully.']);
        } catch (\Exception $e) {
            return response()->json(['success' => false, 'message' => 'Failed to save rent reminder date. Please try again later. Error: ' . $e->getMessage()]);
        }
    }
}
